...
return_data_base_crop <- function(data_to_save){
...
# write.csv(sua_Data,"sua_Data_test.csv",row.names = F)
...
xx[ElementCode %in% unique(data_to_save$ElementCode) & CPCCode %in% unique(subset(classification, classification %in% c("CP","CD","C"))[,CPCCode])
   , `:=` (i.Value = Value, i.Flag= Flag)]
