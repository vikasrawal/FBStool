...
fbs_balanced_plugin <- function(input,output,session){


##check the years before extraction
...
calculateImbalance <- function(data,
                               ...
                               keep_supply = TRUE, #update
                               keep_utilizations = TRUE) {#update
...
#   file.remove(paste0(basedir,"/SUA-FBS Balancing/Data/ShareUpDownTree.csv"))
#   write.csv(ShareUpDownTree,paste0(basedir,"/SUA-FBS Balancing/Data/ShareUpDownTree.csv"),row.names = FALSE)
# }else{
#   write.csv(ShareUpDownTree,paste0(basedir,"/SUA-FBS Balancing/Data/ShareUpDownTree.csv"),row.names = FALSE)
#   
# }

shareUpDownTree <- fread(paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/ShareUpDownTree.csv"))
...
t_share <- as.character(input$endyear)

for (i in c(2010:t_share)){
  
  
  condition_share <- i %in% unique(shareUpDownTree$timePointYears)
  
  if (condition_share == FALSE){
    
    years_missing_share<- c(years_missing_share,i)
...
# 
# if(file.exists(paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/fbs_standardized.csv"))){
#   file.remove(paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/fbs_standardized.csv"))
#   write.csv(fbs_standardized,paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/fbs_standardized.csv"),row.names = FALSE)
# }else{
#   
#   write.csv(fbs_standardized,paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/fbs_standardized.csv"),row.names = FALSE)
# }
...
# 
# 
# if(file.exists(paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/fbs_balancedData.csv"))){
#   file.remove(paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/fbs_balancedData.csv"))
#   write.csv(fbs_balancedData,paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/fbs_balancedData.csv"),row.names = FALSE)
# }else{
#   
#   write.csv(fbs_balancedData,paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/fbs_balancedData.csv"),row.names = FALSE)
# }
...
elementCodesToNames <- function(data, elementCol = NULL, itemCol = NULL, standParams = NULL){
  ...
  elementMap <- fread(paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/elementMap.csv"))
  ...
  itemCodeKey <- fread(paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/itemCodeKey.csv"))
  ...
  itemCodeKey[,production := as.character(production)]
  ...
  out[, measuredElementSuaFbs := as.character(measuredElementSuaFbs)]
...
#flagMethod is removed 
sel_vars <-
  c("measuredItemParentCPC", "geographicAreaM49", "timePointYears",
    "measuredElementSuaFbs", "flagObservationStatus", 
    "Value", "measuredItemChildCPC", "extractionRate"
  )
...
#some correction
fbsTree[, fbsID4 := as.character(fbsID4)]
...
#                   "timePointYears","Value","Official","Protected","flagObservationStatus","flagMethod"))]

data=data[,mget(c("measuredItemSuaFbs","measuredElementSuaFbs", "geographicAreaM49", 
                  "timePointYears","Value","Official","Protected","flagObservationStatus"))]
...
nutrientData[, measuredElementSuaFbs := as.character(measuredElementSuaFbs)]
nutrientData[get(params$elementVar)=="664",params$elementVar:=params$calories]
...
dataDes<-rbind(calories_per_capita,calories_per_capita_total[measuredElementSuaFbs %in% c("TotalCalories","TotalProteins","TotalFats")])
#################################################################
#################################################################
#################################################################
...
parentNodes <- fread(paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/parentNodes.csv"))
...
for (i in seq_len(nrow(uniqueLevels))) {
  ...
  standardization<-function(dataQTY=dataSubset,
    ...
    dataQTY[,c(params$geoVar,params$yearVar):= lapply(.SD, as.character), .SDcols = c(params$geoVar,params$yearVar)]
    ...
    dataDES[, geographicAreaM49  := as.character(geographicAreaM49 )]
    dataDES[, timePointYears  := as.character(timePointYears )]
    ...
    standardizeQty<-standardizeQty[,list(geographicAreaM49,timePointYears,measuredItemParentCPC,
                                         measuredItemChildCPC,measuredElementSuaFbs,
                                         Value,flagObservationStatus,extractionRate,share,
                                         standard_child,weight,Stand_Value=Value/extractionRate*share*weight)]
  ...
  dataSubset[,c(params$geoVar,params$yearVar):= lapply(.SD, as.character), .SDcols = c(params$geoVar,params$yearVar)]
...
# 
# NonStanditemChild <- merge(NonStanditemChild,all_cpc,by.x = c("measuredItemFbsSua"),by.y = c("CPCCode") ,all.x = TRUE)
...
popSWS[,c("geographicAreaM49","measuredElement","timePointYears"):= lapply(.SD, as.character), .SDcols =
         c("geographicAreaM49","measuredElement","timePointYears")]
...
#flag method is removed
foodGram_data<-foodGram_data[,list(geographicAreaM49,measuredItemFbsSua,measuredElementSuaFbs,timePointYears,Value,flagObservationStatus)]
...
if(file.exists(paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/fbs_standardized_final.csv"))){
  file.remove(paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/fbs_standardized_final.csv"))
...
Item_with_unbalanced[,geographicAreaM49 := as.character(geographicAreaM49)]
Item_with_unbalanced[,timePointYears:= as.character(timePointYears)]
...
if(file.exists(paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/fbs_balanced_final.csv"))){
  file.remove(paste0(basedir,"/SUA-FBS Balancing/FBS_Balanced/Data/fbs_balanced_final.csv"))
